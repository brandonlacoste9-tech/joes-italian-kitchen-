...
export function SocialLinks({ tone = 'ink', compact = false }: Props) {
  const chip =
    tone === 'cream'
      ...
      : 'border-ink/30 text-ink hover:border-red hover:text-red';
      ...
      {socials.map((item) => (
        <li key={item.id}>
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className={`inline-flex items-center gap-2 border px-3 py-2 text-[0.72rem] font-bold tracking-[0.12em] uppercase ${chip}`}
